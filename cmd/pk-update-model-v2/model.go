...
type SetWebhook struct { 
    ...
    MaxConnections int `json:"max_connections"` //The maximum allowed number of simultaneous HTTPS connections to the webhook for update delivery, 1-100. Defaults to 40. Use lower values to limit the load on your bot's server, and higher values to increase your bot's throughput.
    AllowedUpdates []string `json:"allowed_updates"` //A JSON-serialized list of the update types you want your bot to receive. For example, specify [“message”, “edited_channel_post”, “callback_query”] to only receive updates of these types. See Update for a complete list of available update types. Specify an empty list to receive all update types except chat_member (default). If not specified, the previous setting will be used.Please note that this parameter doesn't affect updates created before the call to the setWebhook, so unwanted updates may be received for a short period of time.
    DropPendingUpdates bool `json:"drop_pending_updates"` //Pass True to drop all pending updates
    SecretToken string `json:"secret_token"` //A secret token to be sent in a header “X-Telegram-Bot-Api-Secret-Token” in every webhook request, 1-256 characters. Only characters A-Z, a-z, 0-9, _ and - are allowed. The header is useful to ensure that the request comes from a webhook set by you.
}
...
type WebhookInfo struct { 
    ...
    LastSynchronizationErrorDate int `json:"last_synchronization_error_date"` //Optional. Unix time of the most recent error that happened when trying to synchronize available updates with Telegram datacenters
    MaxConnections int `json:"max_connections"` //Optional. The maximum allowed number of simultaneous HTTPS connections to the webhook for update delivery
...
type User struct { 
    Id int `json:"id,omitempty"` //Unique identifier for this user or bot. This number may have more than 32 significant bits and some programming languages may have difficulty/silent defects in interpreting it. But it has at most 52 significant bits, so a 64-bit integer or double-precision float type are safe for storing this identifier.
...
type MessageId struct { 
    MessageId int `json:"message_id,omitempty"` //Unique message identifier
}
 
//MessageEntity This object represents one special entity in a text message. For example, hashtags, usernames, URLs, etc.
type MessageEntity struct { 
    Type string `json:"type,omitempty"` //Type of the entity. Currently, can be “mention” (@username), “hashtag” (#hashtag), “cashtag” ($USD), “bot_command” (/start@jobs_bot), “url” (https://telegram.org), “email” (do-not-reply@telegram.org), “phone_number” (+1-212-555-0123), “bold” (bold text), “italic” (italic text), “underline” (underlined text), “strikethrough” (strikethrough text), “spoiler” (spoiler message), “code” (monowidth string), “pre” (monowidth block), “text_link” (for clickable text URLs), “text_mention” (for users without usernames), “custom_emoji” (for inline custom emoji stickers)
...
type MessageAutoDeleteTimerChanged struct { 
    MessageAutoDeleteTime int `json:"message_auto_delete_time,omitempty"` //New auto-delete time for messages in the chat; in seconds
}
 
//VideoChatScheduled This object represents a service message about a video chat scheduled in the chat.
type VideoChatScheduled struct { 
    StartDate int `json:"start_date,omitempty"` //Point in time (Unix timestamp) when the video chat is supposed to be started by a chat administrator
}
 
//VideoChatEnded This object represents a service message about a video chat ended in the chat.
type VideoChatEnded struct { 
    Duration int `json:"duration,omitempty"` //Video chat duration in seconds
}
...
type InlineKeyboardButton struct { 
    ...
    WebApp *WebAppInfo `json:"web_app"` //Optional. Description of the Web App that will be launched when the user presses the button. The Web App will be able to send an arbitrary message on behalf of the user using the method answerWebAppQuery. Available only in private chats between a user and the bot.
    ...
    Pay bool `json:"pay"` //Optional. Specify True, to send a Pay button.NOTE: This type of button must always be the first button in the first row and can only be used in invoice messages.
}
...
type ChatMemberUpdated struct { 
    ...
    Date int `json:"date,omitempty"` //Date the change was done in Unix time
...
type ChatJoinRequest struct { 
    ...
    Date int `json:"date,omitempty"` //Date the request was sent in Unix time
...
type InputMediaVideo struct { 
    ...
    Media string `json:"media,omitempty"` //File to send. Pass a file_id to send a file that exists on the Telegram servers (recommended), pass an HTTP URL for Telegram to get a file from the Internet, or pass “attach://<file_attach_name>” to upload a new one using multipart/form-data under <file_attach_name> name. More information on Sending Files »
    Thumb string `json:"thumb"` //Optional. Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under <file_attach_name>. More information on Sending Files »
    ...
    Duration int `json:"duration"` //Optional. Video duration in seconds
...
type InputMediaAnimation struct { 
    ...
    Duration int `json:"duration"` //Optional. Animation duration in seconds
}
...
type SendMessage struct { 
    ...
    Entities []MessageEntity `json:"entities"` //A JSON-serialized list of special entities that appear in message text, which can be specified instead of parse_mode
    ...
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding and saving
...
type ForwardMessage struct { 
    ...
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the forwarded message from forwarding and saving
...
type CopyMessage struct { 
    ...
    CaptionEntities []MessageEntity `json:"caption_entities"` //A JSON-serialized list of special entities that appear in the new caption, which can be specified instead of parse_mode
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding and saving
    ReplyToMessageId int `json:"reply_to_message_id"` //If the message is a reply, ID of the original message
    AllowSendingWithoutReply bool `json:"allow_sending_without_reply"` //Pass True if the message should be sent even if the specified replied-to message is not found
...
type SendPhoto struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Photo string `json:"photo,omitempty"` //Photo to send. Pass a file_id as String to send a photo that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a photo from the Internet, or upload a new photo using multipart/form-data. The photo must be at most 10 MB in size. The photo's width and height must not exceed 10000 in total. Width and height ratio must be at most 20. More information on Sending Files »
    ...
    CaptionEntities []MessageEntity `json:"caption_entities"` //A JSON-serialized list of special entities that appear in the caption, which can be specified instead of parse_mode
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding and saving
    ReplyToMessageId int `json:"reply_to_message_id"` //If the message is a reply, ID of the original message
    AllowSendingWithoutReply bool `json:"allow_sending_without_reply"` //Pass True if the message should be sent even if the specified replied-to message is not found
...
type SendAudio struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Audio string `json:"audio,omitempty"` //Audio file to send. Pass a file_id as String to send an audio file that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get an audio file from the Internet, or upload a new one using multipart/form-data. More information on Sending Files »
    ...
    Thumb string `json:"thumb"` //Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under <file_attach_name>. More information on Sending Files »
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding and saving
    ReplyToMessageId int `json:"reply_to_message_id"` //If the message is a reply, ID of the original message
    AllowSendingWithoutReply bool `json:"allow_sending_without_reply"` //Pass True if the message should be sent even if the specified replied-to message is not found
...
type SendDocument struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Document string `json:"document,omitempty"` //File to send. Pass a file_id as String to send a file that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a file from the Internet, or upload a new one using multipart/form-data. More information on Sending Files »
    Thumb string `json:"thumb"` //Thumbnail of the file sent; can be ignored if thumbnail generation for the file is supported server-side. The thumbnail should be in JPEG format and less than 200 kB in size. A thumbnail's width and height should not exceed 320. Ignored if the file is not uploaded using multipart/form-data. Thumbnails can't be reused and can be only uploaded as a new file, so you can pass “attach://<file_attach_name>” if the thumbnail was uploaded using multipart/form-data under <file_attach_name>. More information on Sending Files »
...
type SendVideo struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Video string `json:"video,omitempty"` //Video to send. Pass a file_id as String to send a video that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a video from the Internet, or upload a new video using multipart/form-data. More information on Sending Files »
...
type SendAnimation struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Animation string `json:"animation,omitempty"` //Animation to send. Pass a file_id as String to send an animation that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get an animation from the Internet, or upload a new animation using multipart/form-data. More information on Sending Files »
...
type SendVoice struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Voice string `json:"voice,omitempty"` //Audio file to send. Pass a file_id as String to send a file that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a file from the Internet, or upload a new one using multipart/form-data. More information on Sending Files »
...
type SendVideoNote struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    VideoNote string `json:"video_note,omitempty"` //Video note to send. Pass a file_id as String to send a video note that exists on the Telegram servers (recommended) or upload a new video using multipart/form-data. More information on Sending Files ». Sending video notes by a URL is currently unsupported
    Duration int `json:"duration"` //Duration of sent video in seconds
...
type SendMediaGroup struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Media []InputMediaAudio, InputMediaDocument, InputMediaPhoto and InputMediaVideo `json:"media,omitempty"` //A JSON-serialized array describing messages to be sent, must include 2-10 items
    ...
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent messages from forwarding and saving
...
type SendLocation struct { 
    ...
    HorizontalAccuracy float32 `json:"horizontal_accuracy"` //The radius of uncertainty for the location, measured in meters; 0-1500
...
type EditMessageLiveLocation struct { 
    ...
    ProximityAlertRadius int `json:"proximity_alert_radius"` //The maximum distance for proximity alerts about approaching another chat member, in meters. Must be between 1 and 100000 if specified.
...
type SendPoll struct { 
    ...
    ExplanationEntities []MessageEntity `json:"explanation_entities"` //A JSON-serialized list of special entities that appear in the poll explanation, which can be specified instead of parse_mode
...
type SendDice struct { 
    ...
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding
    ReplyToMessageId int `json:"reply_to_message_id"` //If the message is a reply, ID of the original message
    AllowSendingWithoutReply bool `json:"allow_sending_without_reply"` //Pass True if the message should be sent even if the specified replied-to message is not found
    ReplyMarkup InlineKeyboardMarkup `json:"reply_markup"` //Additional interface options. A JSON-serialized object for an inline keyboard, custom reply keyboard, instructions to remove reply keyboard or to force a reply from the user.
}
...
type PromoteChatMember struct { 
    ...
    CanManageVideoChats bool `json:"can_manage_video_chats"` //Pass True if the administrator can manage video chats
...
type BanChatSenderChat struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    SenderChatId int `json:"sender_chat_id,omitempty"` //Unique identifier of the target sender chat
}
...
type CreateChatInviteLink struct { 
    ...
    MemberLimit int `json:"member_limit"` //The maximum number of users that can be members of the chat simultaneously after joining the chat via this invite link; 1-99999
    CreatesJoinRequest bool `json:"creates_join_request"` //True, if users joining the chat via the link need to be approved by chat administrators. If True, member_limit can't be specified
}
...
type RevokeChatInviteLink struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier of the target chat or username of the target channel (in the format @channelusername)
...
type SetMyCommands struct { 
    ...
    Scope *BotCommandScope `json:"scope"` //A JSON-serialized object, describing scope of users for which the commands are relevant. Defaults to BotCommandScopeDefault.
...
type GetMyCommands struct { 
    Scope *BotCommandScope `json:"scope"` //A JSON-serialized object, describing scope of users. Defaults to BotCommandScopeDefault.
...
type SetChatMenuButton struct { 
    ChatId int `json:"chat_id"` //Unique identifier for the target private chat. If not specified, default bot's menu button will be changed
    MenuButton *MenuButton `json:"menu_button"` //A JSON-serialized object for the bot's new menu button. Defaults to MenuButtonDefault
}
...
type GetChatMenuButton struct { 
    ChatId int `json:"chat_id"` //Unique identifier for the target private chat. If not specified, default bot's menu button will be returned
}
...
type SetMyDefaultAdministratorRights struct { 
    Rights *ChatAdministratorRights `json:"rights"` //A JSON-serialized object describing new default administrator rights. If not specified, the default administrator rights will be cleared.
    ForChannels bool `json:"for_channels"` //Pass True to change the default administrator rights of the bot in channels. Otherwise, the default administrator rights of the bot for groups and supergroups will be changed.
}
...
type GetMyDefaultAdministratorRights struct { 
    ForChannels bool `json:"for_channels"` //Pass True to get default administrator rights of the bot in channels. Otherwise, default administrator rights of the bot for groups and supergroups will be returned.
}
...
type StickerSet struct { 
    ...
    IsVideo bool `json:"is_video,omitempty"` //True, if the sticker set contains video stickers
    Stickers []Sticker `json:"stickers,omitempty"` //List of all set stickers
    Thumb *PhotoSize `json:"thumb"` //Optional. Sticker set thumbnail in the .WEBP, .TGS, or .WEBM format
}
...
type SendSticker struct { 
    ChatId string `json:"chat_id,omitempty"` //Unique identifier for the target chat or username of the target channel (in the format @channelusername)
    Sticker string `json:"sticker,omitempty"` //Sticker to send. Pass a file_id as String to send a file that exists on the Telegram servers (recommended), pass an HTTP URL as a String for Telegram to get a .WEBP file from the Internet, or upload a new one using multipart/form-data. More information on Sending Files »
    DisableNotification bool `json:"disable_notification"` //Sends the message silently. Users will receive a notification with no sound.
    ProtectContent bool `json:"protect_content"` //Protects the contents of the sent message from forwarding and saving
    ReplyToMessageId int `json:"reply_to_message_id"` //If the message is a reply, ID of the original message
    AllowSendingWithoutReply bool `json:"allow_sending_without_reply"` //Pass True if the message should be sent even if the specified replied-to message is not found
...
type GetCustomEmojiStickers struct { 
    CustomEmojiIds []string `json:"custom_emoji_ids,omitempty"` //List of custom emoji identifiers. At most 200 custom emoji identifiers can be specified.
}
...
type UploadStickerFile struct { 
    ...
    PngSticker string `json:"png_sticker,omitempty"` //PNG image with the sticker, must be up to 512 kilobytes in size, dimensions must not exceed 512px, and either width or height must be exactly 512px. More information on Sending Files »
}
...
type CreateNewStickerSet struct { 
    ...
    PngSticker string `json:"png_sticker"` //PNG image with the sticker, must be up to 512 kilobytes in size, dimensions must not exceed 512px, and either width or height must be exactly 512px. Pass a file_id as a String to send a file that already exists on the Telegram servers, pass an HTTP URL as a String for Telegram to get a file from the Internet, or upload a new one using multipart/form-data. More information on Sending Files »
    TgsSticker string `json:"tgs_sticker"` //TGS animation with the sticker, uploaded using multipart/form-data. See https://core.telegram.org/stickers#animated-sticker-requirements for technical requirements
    WebmSticker string `json:"webm_sticker"` //WEBM video with the sticker, uploaded using multipart/form-data. See https://core.telegram.org/stickers#video-sticker-requirements for technical requirements
    StickerType string `json:"sticker_type"` //Type of stickers in the set, pass “regular” or “mask”. Custom emoji sticker sets can't be created via the Bot API at the moment. By default, a regular sticker set is created.
...
type InlineQueryResultGif struct { 
    ...
    GifDuration int `json:"gif_duration"` //Optional. Duration of the GIF in seconds
    ThumbUrl string `json:"thumb_url,omitempty"` //URL of the static (JPEG or GIF) or animated (MPEG4) thumbnail for the result
...
type InlineQueryResultMpeg4Gif struct { 
    ...
    Id string `json:"id,omitempty"` //Unique identifier for this result, 1-64 bytes
    Mpeg4Url string `json:"mpeg4_url,omitempty"` //A valid URL for the MPEG4 file. File size must not exceed 1MB
    Mpeg4Width int `json:"mpeg4_width"` //Optional. Video width
    Mpeg4Height int `json:"mpeg4_height"` //Optional. Video height
    Mpeg4Duration int `json:"mpeg4_duration"` //Optional. Video duration in seconds
    ThumbUrl string `json:"thumb_url,omitempty"` //URL of the static (JPEG or GIF) or animated (MPEG4) thumbnail for the result
    ThumbMimeType string `json:"thumb_mime_type"` //Optional. MIME type of the thumbnail, must be one of “image/jpeg”, “image/gif”, or “video/mp4”. Defaults to “image/jpeg”
    Title string `json:"title"` //Optional. Title for the result
...
type InputInvoiceMessageContent struct { 
    ...
    PhotoSize int `json:"photo_size"` //Optional. Photo size in bytes
    ...
    SendPhoneNumberToProvider bool `json:"send_phone_number_to_provider"` //Optional. Pass True if the user's phone number should be sent to provider
    SendEmailToProvider bool `json:"send_email_to_provider"` //Optional. Pass True if the user's email address should be sent to provider
...
type AnswerWebAppQuery struct { 
    ...
    Result InlineQueryResultVoice `json:"result,omitempty"` //A JSON-serialized object describing the message to be sent
}
...
type SendInvoice struct { 
    ...
    ProviderData string `json:"provider_data"` //JSON-serialized data about the invoice, which will be shared with the payment provider. A detailed description of required fields should be provided by the payment provider.
    ...
    PhotoSize int `json:"photo_size"` //Photo size in bytes
    ...
    SendPhoneNumberToProvider bool `json:"send_phone_number_to_provider"` //Pass True if the user's phone number should be sent to provider
    SendEmailToProvider bool `json:"send_email_to_provider"` //Pass True if the user's email address should be sent to provider
...
type CreateInvoiceLink struct { 
    ...
    SuggestedTipAmounts []int `json:"suggested_tip_amounts"` //A JSON-serialized array of suggested amounts of tips in the smallest units of the currency (integer, not float/double). At most 4 suggested tip amounts can be specified. The suggested tip amounts must be positive, passed in a strictly increased order and must not exceed max_tip_amount.
    ProviderData string `json:"provider_data"` //JSON-serialized data about the invoice, which will be shared with the payment provider. A detailed description of required fields should be provided by the payment provider.
    PhotoUrl string `json:"photo_url"` //URL of the product photo for the invoice. Can be a photo of the goods or a marketing image for a service.
    PhotoSize int `json:"photo_size"` //Photo size in bytes
    PhotoWidth int `json:"photo_width"` //Photo width
    PhotoHeight int `json:"photo_height"` //Photo height
    NeedName bool `json:"need_name"` //Pass True if you require the user's full name to complete the order
    ...
    NeedEmail bool `json:"need_email"` //Pass True if you require the user's email address to complete the order
    NeedShippingAddress bool `json:"need_shipping_address"` //Pass True if you require the user's shipping address to complete the order
    SendPhoneNumberToProvider bool `json:"send_phone_number_to_provider"` //Pass True if the user's phone number should be sent to the provider
    SendEmailToProvider bool `json:"send_email_to_provider"` //Pass True if the user's email address should be sent to the provider
    IsFlexible bool `json:"is_flexible"` //Pass True if the final price depends on the shipping method
}
